...
ORA <- function(top_table,
    ...
    tryCatch({
        ...
        if (isTRUE(topN)){
            top_table_sort <- dplyr::arrange(top_table, `p-value`)
...
performGSEA <- function(top_table,
    ...
    tryCatch({
        ...
        if (which(colnames(top_table) == geneID_col) != 1){
            ...
            # Split all genes by a comma
            genes_all <- stringr::str_split(top_table[,geneID_col], "; ")
        ...
        if (rankingVar == "logFC"){
            temp <- dplyr::arrange(top_table, by = `p-value`)
        ...
        # Order results
        GSEA_data@result <- dplyr::arrange(output, by = `p-value`)
...
make_ORAgene_table <- function(ORA_data,
    ...
    if (which(colnames(top_table) == geneID_col) != 1){
        ...
        # Make separate row names for single gene name
        colnames(top_table)[colnames(top_table) == geneID_col] <- "ID"
        top_table_name <- dplyr::left_join(top_table, gene_names, by = c("ID" = "combined_name"))
        colnames(top_table_name)[colnames(top_table_name) == "ID"] <- geneID_col
        ...
        if (ORA_data@keytype !=  geneID_type){
            ...
            termGenes <- AnnotationDbi::select(BiocGenerics::get(
                pkg, envir = asNamespace(pkg)), 
                columns = geneID_type, 
                keys = termGenes,
                keytype = ORA_data@keytype)[,geneID_type]
        }
