observe({
  ...
  observeEvent(input$upload_rnaseq_norm,{
    ...
    if (!is.null(input$uploadExprData_rnaseq_norm)){
      rv$gxData <- readExprMatrix(path=input$uploadExprData_rnaseq_norm$datapath)
    }else{
  ...
  observeEvent(input$example_rnaseq_norm,{
    ...
    # Read expression data
    rv$gxData <- readExprMatrix(path="Data/RNAseq/normExpr_GSE128380.csv", 
                                checkDup = FALSE)
    ...
    if(nrow(rv$metaData)>0){
      ...
      if (nrow(rv$metaData) != ncol(rv$gxData)){
        shinyWidgets::sendSweetAlert(
          session = session,
          title = "Warning!",
          text = "One or more sample(s) in the expression data file do(e) not have 
  ...
  observeEvent(input$start_preprocessing_rnaseq_norm,{
    ...
    rv$processingSettings <- data.frame(
      ...
      Selected = c(paste(rv$outlier, collapse = "; "),
                   ...
                   paste(input$normMeth_rnaseq_norm, input$perGroup_rnaseq_norm, sep = "; "),
    ...
    # QC report
    #********************************************************************#
    output$QCreport_rnaseq_norm <- downloadHandler(
      ...
      content = function(file) {
        ...
        tempReport <- file.path(tempdir(), "QCreport_RNAseq_norm.Rmd")
        file.copy("Reports/QCreport_RNAseq_norm.Rmd", tempReport, overwrite = TRUE)
      ...
      tagList(
        tabsetPanel(
          ...
          tabPanel("Expression values",
                   ...
                   actionButton("link_geneboxplot_rnaseq_norm", 
                                "Explain figure",
          ...
          tabPanel("Boxplots",
                   ...
                   actionButton("download_boxplots_rnaseq_norm", 
                                "Download figure",
                                icon = shiny::icon("download")),
                   actionButton("link_boxplots_rnaseq_norm", 
                                "Explain figure",
                                icon = shiny::icon("question-circle"),
          ...
          tabPanel("Density plots",
                   ...
                   actionButton("download_density_rnaseq_norm", 
                                "Download figure",
                                icon = shiny::icon("download")),
                   actionButton("link_density_rnaseq_norm", 
                                "Explain figure",
                                icon = shiny::icon("question-circle"),
          ...
          tabPanel("Correlation plot", 
                   ...
                   actionButton('download_heatmap_rnaseq_norm', 
                                'Download figure',
                                icon = shiny::icon("download")),
                   actionButton("link_heatmap_rnaseq_norm", 
                                "Explain figure",
                                icon = shiny::icon("question-circle"),
          ...
          tabPanel("PCA",
                   ...
                   actionButton("download_pca_rnaseq_norm", 
                                "Download figure",
                                icon = shiny::icon("download")),
                   actionButton("link_pca_rnaseq_norm", 
                                "Explain figure",
                                icon = shiny::icon("question-circle"),
    ...
    output$UI_next_preprocessing_rnaseq_norm <- renderUI({
      ...
      tagList(
        hr(),
        h2(strong("Continue your analysis")),
        shinyWidgets::downloadBttn(outputId = "QCreport_rnaseq_norm",
                                   label = "Get report",
                                   style = "jelly",
                                   color = "primary",
                                   icon = icon("download")),
  ...
  observeEvent(input$calculate_statistics_rnaseq_norm,{
    ...
    if (isTRUE(input$addAnnotation_rnaseq_norm)){
      ...
      for (c in 1:length(input$comparisons_rnaseq_norm)){
        rv$statSettings[[c]] <- data.frame(
          ...
          Selected = c(input$comparisons_rnaseq_norm[c],
                       ifelse(is.null(input$covGroups_num_rnaseq_norm), " ",
      ...
      names(rv$statSettings) <- input$comparisons_rnaseq_norm
    ...
    observe({
      ...
      output$top_table_rnaseq_norm <- DT::renderDataTable({
        ...
        if (is.null(input$comparisons_view_rnaseq_norm)){
    ...
    observe({
      ...
      if (!is.null(input$boxplotAll_rnaseq_norm)){
        ...
        if (!input$boxplotAll_rnaseq_norm){
          ...
          rv$newData <- rv$normData[,t %in% (unlist(stringr::str_split(input$comparisons_view_rnaseq_norm, " - ")))]
          rv$newFactor <- factor(as.character(rv$experimentFactor)[t %in% (unlist(stringr::str_split(input$comparisons_view_rnaseq_norm, " - ")))])
        }
      }
    ...
    # Download plot
    observe({
      req(input$statboxplot_file_rnaseq_norm)
      output$realdownload_statboxplot_rnaseq_norm <- downloadHandler(
        filename = ifelse(input$statboxplot_file_rnaseq_norm == "PNG", "GeneBoxplot.png",
                          ifelse(input$statboxplot_file_rnaseq_norm == "PDF", "GeneBoxplot.pdf",
                                 "GeneBoxplot.tif")),
        content = function(file){
    ...
    observe({
      ...
      if (input$comparisons_view_rnaseq_norm %in% names(rv$top_table)){
        ...
        rv$Phistogram <- makePHistogram(P = rv$top_table[[input$comparisons_view_rnaseq_norm]][,"p-value"],
                                        color = input$histogram_color_rnaseq_norm,
                                        bins = input$histogram_bins_rnaseq_norm)
        ...
        # Download plot
        observe({
          req(input$Phistogram_file_rnaseq_norm)
          output$realdownload_Phistogram_rnaseq_norm <- downloadHandler(
            filename = ifelse(input$Phistogram_file_rnaseq_norm == "HTML", "Phistogram.html",
                              ifelse(input$Phistogram_file_rnaseq_norm == "PNG", "Phistogram.png",
                                     ifelse(input$Phistogram_file_rnaseq_norm == "PDF", "Phistogram.pdf",
                                            "Phistogram.tif"))),
            content = function(file){
              
              if (input$Phistogram_file_rnaseq_norm != "HTML"){
                
                # Make volcano plot
                p <- makePHistogram(P = rv$top_table[[input$comparisons_view_rnaseq_norm]][,"p-value"],
                                    color = input$histogram_color_rnaseq_norm,
                                    bins = input$histogram_bins_rnaseq_norm,
                                    static = TRUE)
                ...
                ggplot2::ggsave(plot = p, 
                                filename = file,
                                width = input$width_Phistogram_rnaseq_norm,
                                height = input$height_Phistogram_rnaseq_norm,
                                units = "px")
              } else{
        ...
        # Make modal
        observeEvent(input$download_Phistogram_rnaseq_norm, {
          showModal(modalDialog(
            title = NULL,
            easyClose = TRUE,
            size = "m",
            footer = tagList(
              ...
              fluidRow(
                column(6,
                       conditionalPanel(
                         condition = "input.Phistogram_file_rnaseq_norm!=`HTML`",
                         sliderInput("height_Phistogram_rnaseq_norm", 
                                     "Height",
                                     min = 800, max = 2000,
                                     value = 1200, step = 10,
                                     width = "100%")
                ...
                column(6,
                       conditionalPanel(
                         condition = "input.Phistogram_file_rnaseq_norm!=`HTML`",
                         sliderInput("width_Phistogram_rnaseq_norm", 
                                     "Width",
                                     min = 800, max = 2000,
                                     value = 1500, step = 10,
                                     width = "100%")
              ...
              fluidRow(
                column(12, align = "left",
                       downloadButton('realdownload_Phistogram_rnaseq_norm', 
                                      'Download')
                )
              )
        ...
        rv$logFChistogram <- makelogFCHistogram(logFC = rv$top_table[[input$comparisons_view_rnaseq_norm]][,"log2FC"],
                                                color = input$histogram_color_rnaseq_norm,
                                                bins = input$histogram_bins_rnaseq_norm)
        
        # Plot logFC histogram
        output$logFChistogram_rnaseq_norm <- plotly::renderPlotly({
        ...
        # Download plot
        observe({
          req(input$logFChistogram_file_rnaseq_norm)
          output$realdownload_logFChistogram_rnaseq_norm <- downloadHandler(
            filename = ifelse(input$logFChistogram_file_rnaseq_norm == "HTML", "logFChistogram.html",
                              ifelse(input$logFChistogram_file_rnaseq_norm == "PNG", "logFChistogram.png",
                                     ifelse(input$logFChistogram_file_rnaseq_norm == "PDF", "logFChistogram.pdf",
                                            "logFChistogram.tif"))),
            content = function(file){
              
              if (input$logFChistogram_file_rnaseq_norm != "HTML"){
                
                # Make volcano plot
                p <- makelogFCHistogram(logFC = rv$top_table[[input$comparisons_view_rnaseq_norm]][,"log2FC"],
                                        color = input$histogram_color_rnaseq_norm,
                                        bins = input$histogram_bins_rnaseq_norm,
                                        static = TRUE)
                ...
                ggplot2::ggsave(plot = p, 
                                filename = file,
                                width = input$width_logFChistogram_rnaseq_norm,
                                height = input$height_logFChistogram_rnaseq_norm,
                                units = "px")
              } else{
        ...
        # Make modal
        observeEvent(input$download_logFChistogram_rnaseq_norm, {
          showModal(modalDialog(
            title = NULL,
            easyClose = TRUE,
            size = "m",
            footer = tagList(
              ...
              fluidRow(
                column(6,
                       conditionalPanel(
                         condition = "input.logFChistogram_file_rnaseq_norm!=`HTML`",
                         sliderInput("height_logFChistogram_rnaseq_norm", 
                                     "Height",
                                     min = 800, max = 2000,
                                     value = 1200, step = 10,
                                     width = "100%")
                ...
                column(6,
                       conditionalPanel(
                         condition = "input.logFChistogram_file_rnaseq_norm!=`HTML`",
                         sliderInput("width_logFChistogram_rnaseq_norm", 
                                     "Width",
                                     min = 800, max = 2000,
                                     value = 1500, step = 10,
                                     width = "100%")
              ...
              fluidRow(
                column(12, align = "left",
                       downloadButton('realdownload_logFChistogram_rnaseq_norm', 
                                      'Download')
                )
              )
    ...
    # TAB5: Summary
    #********************************************************************#
    observeEvent(input$get_summary_rnaseq_norm, {
      req(rv$top_table)
      req(input$rawp_summary_rnaseq_norm)
      req(input$p_thres_summary_rnaseq_norm)
      req(input$logFC_thres_summary_rnaseq_norm)
      req(input$comparisons_view_rnaseq_norm)
      ...
      if (input$comparisons_view_rnaseq_norm %in% names(rv$top_table)){
        ...
        if (input$rawp_summary_rnaseq_norm == "raw"){
          rownames(temp) <- c(paste0("p-value < ", input$p_thres_summary_rnaseq_norm),
                              paste0("p-value > ", input$p_thres_summary_rnaseq_norm))
          colnames(temp) <- c(paste0("log2FC < -", input$logFC_thres_summary_rnaseq_norm),
                              paste0("-",input$logFC_thres_summary_rnaseq_norm, " < log2FC < ", input$logFC_thres_summary_rnaseq_norm),
        ...
        output$summaryTable_rnaseq_norm <- DT::renderDataTable({
    ...
    # Download button
    output$downloadSummaryTable_rnaseq_norm <- downloadHandler(
    ...
    observe({
      ...
      # Print table with settings
      output$statSettings_rnaseq_norm <- DT::renderDataTable({
        return(rv$statSettings[[input$comparisons_view_rnaseq_norm]])
      },options = list(pageLength = 10,
                       dom = 't',
                       rownames = FALSE),
      selection = "none")
      ...
      # Download button
      output$downloadStatSettings_rnaseq_norm <- downloadHandler(
        filename = "StatisticalAnalysis_Settings.csv",
        content = function(file){
          write.csv(rv$statSettings[[input$comparisons_view_rnaseq_norm]], file, quote = FALSE, row.names = FALSE)
        }
      )
    ...
    #********************************************************************#
    output$SAreport_rnaseq_norm <- downloadHandler(
      # For PDF output, change this to "report.pdf"
      ...
      content = function(file) {
        ...
        # case we don't have write permissions to the current working dir (which
        # can happen when deployed).
        tempReport <- file.path(tempdir(), "SAreport_RNAseq_norm.Rmd")
        file.copy("Reports/SAreport_RNAseq_norm.Rmd", tempReport, overwrite = TRUE)
        ...
        # Set up parameters to pass to Rmd document
        params <- list(statSettings = rv$statSettings[[input$comparisons_view_rnaseq_norm]],
                       topTable = rv$top_table[[input$comparisons_view_rnaseq_norm]],
      ...
      output$UI_boxplotAll_rnaseq_norm <- renderUI({
        tagList(
          tags$div(class = "dropdown-content",
                   ...
                   # This is used when there are more than 6 experimental groups
                   conditionalPanel(
                     condition = "output.length_statboxplot_rnaseq_norm > 6",
            ...
            tabsetPanel(
              ...
              tabPanel("Top table",
                       ...
                       actionButton("download_statboxplot_rnaseq_norm", 
                                    "Download figure",
                                    icon = shiny::icon("download")),
                       actionButton("link_statboxplot_rnaseq_norm", 
                                    "Explain figure",
                                    icon = shiny::icon("question-circle"),
                                    onclick ="window.open('https://arrayanalysis.org/explain/Geneboxplot', '_blank')"),
                       br(),
              ...
              tabPanel("Histograms",
                       ...
                       tags$div(class = "dropdown",
                                ...
                                tags$div(class = "dropdown-content",
                                         ...
                                         colourpicker::colourInput("histogram_color_rnaseq_norm", 
                                                                   NULL, 
                                                                   "#d3d3d3"),
                                         br(),
                                         tags$h4("Number of bins"),
                                         numericInput(inputId = "histogram_bins_rnaseq_norm",
                                                      label = NULL,
                                                      value = 100),
                                         br(),br()
                                )), # EO dropdownButton
                       plotly::plotlyOutput("Phistogram_rnaseq_norm")%>% 
                         shinycssloaders::withSpinner(color="#0dc5c1"),
              ...
              tabPanel("Volcano plot",
                       ...
                       tags$div(class = "dropdown",
                                ...
                                tags$div(class = "dropdown-content",
                                         ...
                                         colourpicker::colourInput("volcano_unchanged_color_rnaseq_norm", 
                                                                   NULL, 
                                                                   "darkgrey"),
                                         tags$h4("Color of downregulated genes"),
                                         colourpicker::colourInput("volcano_down_color_rnaseq_norm", 
                                                                   NULL, 
                                                                   "blue"),
                                         tags$h4("Color of upregulated genes"),
                                         colourpicker::colourInput("volcano_up_color_rnaseq_norm", 
                                                                   NULL, 
                                                                   "red"),
                                         br()
                                )), # EO dropdownButton
                       ...
                       # Volcano plot output
                       plotly::plotlyOutput("volcano_rnaseq_norm")%>% 
                         withSpinner(color="#0dc5c1"),
                       ...
                       fluidRow(
                         column(2,
                                ...
                                shinyWidgets::actionBttn(inputId = "plot_volcano_rnaseq_norm", 
                                                         label = "Load",
                                                         style = "jelly",
                                                         color = "primary",
                                                         icon = icon("sync"))
              ...
              tabPanel("MA plot",
                       ...
                       tags$div(class = "dropdown",
                                ...
                                tags$div(class = "dropdown-content",
                                         tags$h4("Color of unchanged genes"),
                                         colourpicker::colourInput("MA_unchanged_color_rnaseq_norm", 
                                                                   NULL, 
                                                                   "darkgrey"),
                                         tags$h4("Color of downregulated genes"),
                                         colourpicker::colourInput("MA_down_color_rnaseq_norm", 
                                                                   NULL, 
                                                                   "blue"),
                                         tags$h4("Color of upregulated genes"),
                                         colourpicker::colourInput("MA_up_color_rnaseq_norm", 
                                                                   NULL, 
                                                                   "red"),
                                         br()
                                )), # EO dropdownButton
                       ...
                       # MA plot output
                       plotly::plotlyOutput("MA_rnaseq_norm")%>% 
                         withSpinner(color="#0dc5c1"),
                       br(),
                       ...
                       fluidRow(
                         column(2,
                                br(),
                                # Button to reload the volcano plot
                                shinyWidgets::actionBttn(inputId = "plot_MA_rnaseq_norm", 
                                                         label = "Load",
                                                         style = "jelly",
                                                         color = "primary",
                                                         icon = icon("sync"))
              ...
              tabPanel("Summary",
                       ...
                       hr(),
                       DT::dataTableOutput(outputId = "summaryTable_rnaseq_norm") %>%
                         withSpinner(color="#0dc5c1"),
                       br(),
                       downloadButton("downloadSummaryTable_rnaseq_norm",
                                      "Download table"),
                       ...
                       fluidRow(
                         ...
                         column(3,
                                #P value threshold
                                numericInput(
                                  inputId = "p_thres_summary_rnaseq_norm",
                                  label = "P threshold",
                                  value = 0.05,
                                  width = "80%")
                         )
                       ...
                       fluidRow(
                         column(2,
                                br(),
                                # Button to reload the volcano plot
                                shinyWidgets::actionBttn(inputId = "get_summary_rnaseq_norm", 
                                                         label = "Calculate",
                                                         style = "jelly",
                                                         color = "primary",
                                                         icon = icon("sync"))
                         ...
                         column(3,
                                #logFC threshold
                                numericInput(
                                  inputId = "logFC_thres_summary_rnaseq_norm",
                                  label = HTML(paste0("log", tags$sub("2"),"FC threshold")),
                                  value = 1,
                                  width = "80%")
                         )
              ...
              tabPanel("Settings overview",
                       ...
                       hr(),
                       DT::dataTableOutput(outputId = "statSettings_rnaseq_norm") %>% 
                         withSpinner(color="#0dc5c1"),
                       br(),
    ...
    output$UI_next_statistics_rnaseq_norm <- renderUI({
      ...
      tagList(
        hr(),
        h2(strong("Continue your analysis")),
        shinyWidgets::downloadBttn(outputId = "SAreport_rnaseq_norm",
                                   label = "Get report",
                                   style = "jelly",
                                   color = "primary",
                                   icon = icon("download")),
  ...
  observeEvent(input$calculate_ORA_rnaseq_norm,{
    ...
    if (input$ORA_or_GSEA_rnaseq_norm == "ORA"){
      # Show modal
      ...
      if (input$geneset_ORA_rnaseq_norm == "WikiPathways"){
        ...
        pkg <- switch(input$organism_ORA_rnaseq_norm,
      ...
      if (input$topNorThres_rnaseq_norm == "Threshold"){
        rv$ORA_list <- ORA(top_table = rv$top_table[[input$comparisons_view_ORA_rnaseq_norm]],
        ...
        rv$ORA_settings <- data.frame(
          ...
          Selected = c(input$comparisons_view_ORA_rnaseq_norm,
                       paste0(input$geneset_ORA_rnaseq_norm, " (", rv$GeneSetVersion, ")"),
                       paste0(input$selID_ORA_rnaseq_norm, " (top table column: ", input$geneID_ORA_rnaseq_norm, ")"),
                       input$organism_ORA_rnaseq_norm,
                       paste0("ORA (", input$topNorThres_rnaseq_norm, "; ", input$updown_ORA_rnaseq_norm, ")"),
                       paste0(input$p_thres_ORA_rnaseq_norm, " (", input$rawp_ORA_rnaseq_norm, ")"),
                       input$logFC_thres_ORA_rnaseq_norm
          )
        )
      ...
      observe({
        ...
        if (rv$ORA_error){
          ...
          # Make modal
          observeEvent(input$download_ORAplot_rnaseq_norm, {
            showModal(modalDialog(
              title = NULL,
              easyClose = TRUE,
              size = "m",
              footer = tagList(
                ...
                fluidRow(
                  column(12, align = "left",
                         downloadButton('realdownload_ORAplot_rnaseq_norm', 
                                        'Download')
                  )
                )
          ...
          observe({
            ...
            rv$ORAnetwork <- makeORAnetwork(ORA_data = rv$ORA_data,
                                            layout = input$layout_ORAnetwork_rnaseq_norm,
                                            nSets = input$nSets_ORAnetwork_rnaseq_norm,
                                            color = input$color_ORAnetwork_rnaseq_norm)
            
            output$ORAnetwork_rnaseq_norm <- renderPlot(rv$ORAnetwork,
                                                        height = 500, 
                                                        width = 800)
          ...
          # Make modal
          observeEvent(input$download_ORAnetwork_rnaseq_norm, {
            showModal(modalDialog(
              title = NULL,
              easyClose = TRUE,
              size = "m",
              footer = tagList(
                ...
                fluidRow(
                  ...
                  column(6,
                         sliderInput("width_ORAnetwork_rnaseq_norm", 
                                     "Width",
                                     min = 800, max = 2000,
                                     value = 1500, step = 10,
                                     width = "100%")
                ...
                fluidRow(
                  column(12, align = "left",
                         downloadButton('realdownload_ORAnetwork_rnaseq_norm', 
                                        'Download')
                  )
                )
          ...
          observe({
            ...
            # Print table with settings
            output$ORASettings_rnaseq_norm <- DT::renderDataTable({
            ...
            # Download button
            output$downloadORASettings_rnaseq_norm <- downloadHandler(
          ...
          output$ORAreport_rnaseq_norm <- downloadHandler(
            # For PDF output, change this to "report.pdf"
            ...
            content = function(file) {
              ...
              # case we don't have write permissions to the current working dir (which
              # can happen when deployed).
              tempReport <- file.path(tempdir(), "ORAreport_RNAseq_norm.Rmd")
              file.copy("Reports/ORAreport_RNAseq_norm.Rmd", tempReport, overwrite = TRUE)
      ...
      observe({
            ...
            tagList(
              tabsetPanel(
                ...
                tabPanel("Statistics",
                         ...
                         # Statistics table
                         DT::dataTableOutput(outputId = "ORA_table_rnaseq_norm") %>% 
                           shinycssloaders::withSpinner(color="#0dc5c1"),
                         ...
                         # Gene table
                         DT::dataTableOutput(outputId = "ORAgene_table_rnaseq_norm") %>% 
                           shinycssloaders::withSpinner(color="#0dc5c1")
                ),
                ...
                tabPanel("Bar chart",
                         ...
                         tags$div(class = "dropdown",
                                  ...
                                  tags$div(class = "dropdown-content",
                                           ...
                                           selectInput(inputId = "color_ORAplot_rnaseq_norm",
                                                       label = NULL,
                                                       choices = c("Viridis", 
                                                                   "Yellow-red", 
                                                                   "Blues", 
                         ...
                         plotly::plotlyOutput("ORAplot_rnaseq_norm") %>% 
                           shinycssloaders::withSpinner(color="#0dc5c1")
                ),
                ...
                tabPanel("Network diagram",
                         ...
                         tags$div(class = "dropdown",
                                  ...
                                  tags$div(class = "dropdown-content",
                                           ...
                                           # Color gradient
                                           tags$h4("Color gradient"),
                                           selectInput(inputId = "color_ORAnetwork_rnaseq_norm",
                                                       label = NULL,
                                                       choices = c("Viridis", 
                                                                   "Yellow-red", 
                                                                   "Blues", 
                                           ...
                                           selectInput(inputId = "layout_ORAnetwork_rnaseq_norm",
                                                       label = NULL,
                                           ...
                                           tags$h4("# Gene sets"),
                                           sliderInput(
                                             inputId = "nSets_ORAnetwork_rnaseq_norm",
                                             label = NULL,
                                             value = 10,
                                             min = 5,
                                             max = 20,
                ...
                tabPanel("Settings overview",
                         ...
                         hr(),
                         DT::dataTableOutput(outputId = "ORASettings_rnaseq_norm") %>% 
                           withSpinner(color="#0dc5c1"),
                         br(),
        ...
        output$UI_ORAreport_rnaseq_norm <- renderUI({
          req(rv$ORA_data)
          tagList(
            shinyWidgets::downloadBttn(outputId = "ORAreport_rnaseq_norm",
    ...
    if (input$ORA_or_GSEA_rnaseq_norm == "GSEA"){
      ...
      # Perform GSEA:
      rv$GSEA_list <- performGSEA(top_table = rv$top_table[[input$comparisons_view_ORA_rnaseq_norm]],
                                  geneset = input$geneset_ORA_rnaseq_norm,
                                  geneID_col = input$geneID_ORA_rnaseq_norm,
                                  geneID_type = input$selID_ORA_rnaseq_norm,
                                  organism = input$organism_ORA_rnaseq_norm,
                                  rankingVar = input$ranking_GSEA_rnaseq_norm)
      
      rv$GSEA_data <- rv$GSEA_list[["data"]]
      rv$GSEA_error <- rv$GSEA_list[["error"]]
      
      if (input$ranking_GSEA_rnaseq_norm == "pvalue"){
        rankvar <- "-log p-value"
      }
      if (input$ranking_GSEA_rnaseq_norm == "signed_pvalue"){
        rankvar <- "-log p-value x sign logFC"
      }
      if (input$ranking_GSEA_rnaseq_norm == "logFC"){
        rankvar <- "logFC"
      }
      ...
      observe({
        ...
        if (rv$GSEA_error){
          ...
          # Download button
          output$download_GSEA_table_rnaseq_norm <- downloadHandler(
            filename = paste0("GSEATable_",input$comparisons_view_ORA_rnaseq_norm,"_",input$geneset_ORA_rnaseq_norm,".csv"),
            content = function(file){
          ...
          # Make modal
          observeEvent(input$download_GSEAplot_rnaseq_norm, {
            showModal(modalDialog(
              title = NULL,
              easyClose = TRUE,
              size = "m",
              footer = tagList(
                ...
                fluidRow(
                  column(12, align = "left",
                         downloadButton('realdownload_GSEAplot_rnaseq_norm', 
                                        'Download')
                  )
                )
          ...
          observe({
            ...
            rv$GSEAnetwork <- makeGSEAnetwork(GSEA_data = rv$GSEA_data,
                                              layout = input$layout_GSEAnetwork_rnaseq_norm,
                                              nSets = input$nSets_GSEAnetwork_rnaseq_norm,
                                              color = c(input$lowcol_GSEAnetwork_rnaseq_norm,
                                                        input$midcol_GSEAnetwork_rnaseq_norm,
          ...
          # Make modal
          observeEvent(input$download_GSEAnetwork_rnaseq_norm, {
            showModal(modalDialog(
              title = NULL,
              easyClose = TRUE,
              size = "m",
              footer = tagList(
                ...
                fluidRow(
                  ...
                  column(6,
                         sliderInput("width_GSEAnetwork_rnaseq_norm", 
                                     "Width",
                                     min = 800, max = 2000,
                                     value = 1500, step = 10,
                                     width = "100%")
                ...
                fluidRow(
                  column(12, align = "left",
                         downloadButton('realdownload_GSEAnetwork_rnaseq_norm', 
                                        'Download')
                  )
                )
          ...
          observe({
            ...
            # Print table with settings
            output$GSEASettings_rnaseq_norm <- DT::renderDataTable({
            ...
            # Download button
            output$downloadGSEASettings_rnaseq_norm <- downloadHandler(
          ...
          output$GSEAreport_rnaseq_norm <- downloadHandler(
            # For PDF output, change this to "report.pdf"
            ...
            content = function(file) {
              ...
              # case we don't have write permissions to the current working dir (which
              # can happen when deployed).
              tempReport <- file.path(tempdir(), "GSEAreport_RNAseq_norm.Rmd")
              file.copy("Reports/GSEAreport_RNAseq_norm.Rmd", tempReport, overwrite = TRUE)
      ...
      observe({
        if (!is.null(rv$GSEA_data)){
          output$UI_output_GSEA_rnaseq_norm <- renderUI({
            tagList(
              tabsetPanel(
                ...
                tabPanel("Statistics",
                         ...
                         # Statistics table
                         DT::dataTableOutput(outputId = "GSEA_table_rnaseq_norm") %>%
                           shinycssloaders::withSpinner(color="#0dc5c1"),
                         ...
                         # Title + description of gene table
                         htmlOutput("text_GSEAgene_table_rnaseq_norm"),
                         h5("Here you can view the differential expression results for the selected gene set."),
                         hr(),
                         
                         # Gene table
                         DT::dataTableOutput(outputId = "GSEAgene_table_rnaseq_norm") %>%
                           shinycssloaders::withSpinner(color="#0dc5c1")
                ),
                ...
                tabPanel("Bar chart",
                         ...
                         tags$div(class = "dropdown",
                                  ...
                                  tags$div(class = "dropdown-content",
                                           ...
                                           # Color gradient
                                           tags$h4("Color gradient"),
                                           colourpicker::colourInput("lowcol_GSEAplot_rnaseq_norm",
                                                                     NULL,
                                                                     "#000072"),
                                           colourpicker::colourInput("midcol_GSEAplot_rnaseq_norm",
                                                                     NULL,
                                                                     "#FEE6CE"),
                                           colourpicker::colourInput("highcol_GSEAplot_rnaseq_norm",
                                                                     NULL,
                                                                     "red"),
                                           br(),
                                           ...
                                           tags$h4("# Gene sets"),
                                           sliderInput(
                                             inputId = "nSets_GSEAplot_rnaseq_norm",
                                             label = NULL,
                                             value = 10,
                                             min = 5,
                                             max = 20,
                         ...
                         # Interactive plot output
                         plotly::plotlyOutput("GSEAplot_rnaseq_norm") %>%
                           shinycssloaders::withSpinner(color="#0dc5c1")
                ),
                ...
                tabPanel("Network diagram",
                         ...
                         tags$div(class = "dropdown",
                                  ...
                                  tags$div(class = "dropdown-content",
                                           ...
                                           # Color gradient
                                           tags$h4("Color gradient"),
                                           colourpicker::colourInput("lowcol_GSEAnetwork_rnaseq_norm",
                                                                     NULL,
                                                                     "#000072"),
                                           colourpicker::colourInput("midcol_GSEAnetwork_rnaseq_norm",
                                                                     NULL,
                                                                     "#FEE6CE"),
                                           colourpicker::colourInput("highcol_GSEAnetwork_rnaseq_norm",
                                                                     NULL,
                                                                     "red"),
                                           br(),
                                           ...
                                           # Network layout
                                           tags$h4("Network layout"),
                                           selectInput(inputId = "layout_GSEAnetwork_rnaseq_norm",
                                                       label = NULL,
                                                       choices = c('star', 'circle', 'gem', 'dh', 'graphopt', 'grid', 'mds',
                                                                   'randomly', 'fr', 'kk', 'drl', 'lgl'),
                                                       selected = 'graphopt',
                                           ...
                                           tags$h4("# Gene sets"),
                                           sliderInput(
                                             inputId = "nSets_GSEAnetwork_rnaseq_norm",
                                             label = NULL,
                                             value = 10,
                                             min = 5,
                                             max = 20,
                         ...
                         # Make plot
                         plotOutput("GSEAnetwork_rnaseq_norm") %>%
                           shinycssloaders::withSpinner(color="#0dc5c1")
                ),
                ...
                tabPanel("Settings overview",
                         ...
                         hr(),
                         DT::dataTableOutput(outputId = "GSEASettings_rnaseq_norm") %>% 
                           withSpinner(color="#0dc5c1"),
                         br(),
        ...
        output$UI_GSEAreport_rnaseq_norm <- renderUI({
          req(rv$GSEA_data)
          tagList(
            shinyWidgets::downloadBttn(outputId = "GSEAreport_rnaseq_norm",
